...
public class ServerInstance implements Runnable {
	...
	private final ServerInfo proxyServerInfo;
	...
	public ServerInstance(ServerManager manager, ClientServer server) {
		...
		ServerInfo serverInfo = this.proxy.getServerInfo(this.getName());
		if (serverInfo != null) {
			this.proxyServerInfo = serverInfo;
		} else {
			this.proxyServerInfo = this.proxy.constructServerInfo(
					this.getName(),
					new InetSocketAddress(allocation.getIP(), allocation.getPortInt()),
					"",
					false);
			this.proxy.getServers().put(this.getName(), this.proxyServerInfo);
		}
...
//			}
	...
	public void close() {
		...
		this.proxy.getServers().remove(this.getName());
		this.unsubscribe();
	}
